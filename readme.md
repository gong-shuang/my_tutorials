# ROS 2 教程目录说明

## 说明
1. beginner，intermediate， Advanced 分别对应官网教程的内容

## 目录结构

本文件说明 `beginner` 和 `intermediate` 目录下的子目录及其功能。

## beginner 目录
对应官网教程的 beginner：Client libraries 章节

### 1. my_package
| 备注 | 说明 |
|------|------|
| 创建包 | 展示 ROS 2 包的基本结构和测试方法 |

### 2. cpp_pubsub
| 备注 | 说明 |
|------|------|
| C++ 发布与订阅 | 展示基本的消息发布和订阅功能 |

### 3. py_pubsub
| 备注 | 说明 |
|------|------|
| Python 发布与订阅 | 展示基本的消息发布和订阅功能 |

### 4. cpp_srvcli
| 备注 | 说明 |
|------|------|
| C++ 服务客户 | 展示基本的服务调用功能 |

### 5. py_srvcli
| 备注 | 说明 |
|------|------|
| Python 服务客户 | 展示基本的服务调用功能 |

### 6. tutorial_interfaces
| 备注 | 说明 |
|------|------|
| 自定义消息和服务 | 教程中使用的接口定义，包含消息和服务类型 |

### 7. more_interfaces
| 备注 | 说明 |
|------|------|
| 自定于接口 | 展示如何创建和使用自定义消息类型 |

### 8. cpp_parameters
| 备注 | 说明 |
|------|------|
| C++ 带参数的类 | 展示如何在 C++ 节点中使用参数服务器 |

### 9. python_parameters
| 备注 | 说明 |
|------|------|
| Python 带参数的类 | 展示如何在 Python 节点中使用参数 |

### 10. polygon_base
| 备注 | 说明 |
|------|------|
| 插件接口 | 定义多边形面积计算的基类 |

### 11. polygon_plugins
| 备注 | 说明 |
|------|------|
| 插件实现 | 提供具体的多边形面积计算插件 |

## intermediate 目录
对应官网教程的 intermediate 章节

### 1. action_tutorials_interfaces
| 备注 | 说明 |
|------|------|
| 创建action | 包含 Fibonacci 动作类型 |

### 2. action_tutorials_cpp
| 备注 | 说明 |
|------|------|
| C++ action服务与客户 | 展示动作系统的使用 |

### 3. action_tutorials_python （未完成）
| 备注 | 说明 |
|------|------|
| python action服务与客户 | 展示动作系统的使用 |

### 4. composable_node_cpp （未完成）
| 备注 | 说明 |
|------|------|
| C++ 可组合的节点| 未完成 |

### 5. node_interfaces_template_class_cpp （未完成）
| 备注 | 说明 |
|------|------|
| C++ 节点接口模版类 | 未完成 |

### 6. cpp_parameter_event_handler
| 备注 | 说明 |
|------|------|
| C++ 监控参数变化 | 展示如何监听参数变化 |

### 7. launch
| 备注 | 说明 |
|------|------|
| 创建launch文件 | 不是ros2的package，直接使用ros2 launch命令启动py文件 |

### 8. py_launch_example
| 备注 | 说明 |
|------|------|
| 集成launch文件 | 展示 Python 启动文件的编写 |

### 9. launch_tutorial
| 备注 | 说明 |
|------|------|
| Using substitutions | 展示如何在 launch文件中使用变量替换参数 |
| Using event handlers | 展示如何在 launch文件中使用事件处理函数 |
| Managing large projects | 展示如何管理大型项目 |

### 10. learning_tf2_py
| 备注 | 说明 |
|------|------|
| Python 示例 | 展示坐标变换系统的使用 |

### 11. learning_tf2_cpp
| 备注 | 说明 |
|------|------|
| C++ 示例 | 展示坐标变换系统的使用 |
| tf2_ros::MessageFilter | 展示如何在 C++ 中过滤消息，使用 tf2_ros 框架进行坐标变换。 |

### 12. my_math
| 备注 | 说明 |
|------|------|
| Testing | 展示如何测试自定义库的功能 |


### 13. urdf_tutorial
代码来自github：https://github.com/ros/urdf_tutorial
| 备注 | 说明 |
|------|------|
| Building a visual robot model | 通过6个例子展示如何创建和使用机器人模型 |
| Building a movable robot model | 三种常用的有可移动关节：continuous、revolute and prismatic，并通过RViz软件，控制并展示关节的运动 |
| Adding physical and collision properties | 如何添加关节（link）的碰撞和惯性属性 |
| Using Xacro to clean up your code | 如何使用Xacro包来简化代码，提高可维护性 |

### 14. urdf_sim_tutorial
代码来自github：https://github.com/ros/urdf_sim_tutorial
| 备注 | 说明 |
|------|------|
| Using a URDF in Gazebo | 如何将你的URDF机器人模型导入Gazebo仿真环境，并让它能够动起来 |